...
class SheetsVerifier:
    ...
    def get_last_working_day(self):
        ...
        while check_date.weekday() in [5, 6] or self.is_holiday(
            check_date
        ):  # Saturday=5, Sunday=6
        ...
        while i < len(sheet_data):
            ...
            if target_date in date_cell or date_cell == target_date:
                ...
                for j in range(i, min(i + rows_to_check, len(sheet_data))):
                    ...
                    if (
                        j > i
                        and len(str(task_row[1]).strip()) > 0
                        and any(char.isdigit() for char in str(task_row[1]))
                    ):
                        # This looks like a new date in column B, stop processing
                        print(
                            f"  Found new date at row {j + 1}, stopping task collection"
                        )
                        break
                    ...
                    entry = {
                        ...
                        "module_area": (
                            task_row[2] if len(task_row) > 2 else ""
                        ),  # Column C
                        "task_details": (
                            task_row[3] if len(task_row) > 3 else ""
                        ),  # Column D
                        "status": task_row[4] if len(task_row) > 4 else "",  # Column E
                        "activity_type": (
                            task_row[5] if len(task_row) > 5 else ""
                        ),  # Column F
                        "start_time": (
                            task_row[6] if len(task_row) > 6 else ""
                        ),  # Column G
                        "end_time": (
                            task_row[7] if len(task_row) > 7 else ""
                        ),  # Column H
                        "total_duration": (
                            task_row[9] if len(task_row) > 9 else ""
                        ),  # Column J
                        "remarks": (
                    ...
                    if entry["task_details"].strip() or entry["module_area"].strip():
                        ...
                        print(
                            f"  Added task from row {j + 1}: {entry['task_details'][:50]}..."
                        )
                    elif (
                        entry["start_time"].strip()
                        and entry["end_time"].strip()
                        and target_entries
                    ):
                        ...
                        print(
                            f"  Added additional time entry from row {j + 1} for previous task: {previous_entry['task_details'][:50]}..."
                        )
...
"""
        ...
        completed_tasks = len(
            [
                ...
                if "completed" in str(e.get("status", "")).lower()
            ]
        )
...
"""
    ...
    def generate_analysis_message(self, employee_name, last_day_entries):
        ...
        for entry in last_day_entries:
            ...
            activity_groups[activity_type].append(
                {"description": description, "duration": task_duration}
            )
    ...
    def verify_all_employees(self):
        ...
        for engineer_name in ENGINEER_NAMES:
            ...
            last_day_entries = self.parse_last_working_day_entries(
                sheet_data, last_working_day
            )

            print(
                f"Found {len(last_day_entries)} entries for {engineer_name} on {last_working_day}"
            )
            if last_day_entries:
                # Analyze the data
                analysis = self.generate_analysis_message(
                    engineer_name, last_day_entries
                )
